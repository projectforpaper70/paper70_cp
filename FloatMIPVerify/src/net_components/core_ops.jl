...
#! format: off
#here the lower_bound_type is key ,the lower_bound is value related
bound_f = Dict(
...
"""#here the lower_bound_type is key ,the lower_bound is value rel
$(SIGNATURES)
...
function tight_bound(
    ...
    #println("the interval compute x is : \n")
    b_0 = bound_f[bound_type](x)
    # println("b_0 in :",bound_type,"  ",b_0)
...
@enum ReLUType split1 = 0 zero_output = -1 linear_in_input = 1 constant_output = 2
...
##这里添加sage求解相关的maxmum_itv
function maximum_itv(
    sgModel::PyObject,
    xs::AbstractArray{T},
    xs_itv::Array{PyObject,1}
    )::PyObject where {T<:JuMPLinearType}   
    ...
    if l == u
        x_itv_rect = sgModel.new_variable()
        sgModel.add_constraint(x_itv_rect[0] == l)
        return x_itv_rect[0]
        Memento.info(MIPVerify.LOGGER, "Output of maximum is constant.")
    end
    # at least one index will satisfy this property because of check above.
    ...
    return maximum_itv(sgModel, xs[filtered_indexes], xs_itv[filtered_indexes], ls[filtered_indexes], us[filtered_indexes])
end
...
function maximum_itv(
    ...
    @assert length(xs_itv) > 0
    @assert length(xs_itv) == length(ls)
    @assert length(xs_itv) == length(us)
    ...
    if length(xs_itv) == 1
        ...
        l = Base.maximum(ls)
        u = Base.maximum(us)
        x_max = sgModel.new_variable()
        sgModel.set_max(x_max[1],u)
        sgModel.set_min(x_max[1],l)
        #x_max = @variable(model, lower_bound = l, upper_bound = u)

        a_var = sgModel.new_variable(binary=true)
        a = [a_var[i] for i in 1:length(xs_itv)]
        sgModel.add_constraint(sgModel.sum(a)==1)

        for(i,x) in enumerate(xs_itv)
            umaxi = Base.maximum(us[1:end.!=i])
            sgModel.add_constraint(x_max[1] <= x + (1 - a[i]) * (umaxi - ls[i]))
            sgModel.add_constraint(x_max[1] >= x)
        end
        return x_max[1]
    end
end
...
#添加sagemath优化目标变量
function maximum_ge_itv(
    sgModel::PyObject,
    x_itv::AbstractArray{PyObject}
)::PyObject
    @assert length(x_itv)>0
    x_max = sgModel.new_variable()
    for i in 1:size(x_itv,1)
        sgModel.add_constraint(x_max[1]>=x_itv[i])
    end
    return x_max[1]
end
...
function abs_ge_itv(
    ...
    # if is_constant(x)
    #     x_itv_abs = sgModel.new_variable()
    #     sgModel.add_constraint(x_itv_abs[1]==abs(x.constant))
    #     return x_itv_abs[1]
    # end

    u = sgModel.get_max(x_itv)
    l = sgModel.get_min(x_itv)

    if u<=0
        x_itv_abs = sgModel.new_variable()
        sgModel.add_constraint(x_itv_abs[1]==-x_itv)
        return x_itv_abs[1]
    elseif l>=0
        return x_itv
    else
        ...
        sgModel.add_constraint(x_itv_abs[1]>=x_itv)
        sgModel.add_constraint(x_itv_abs[1]>=-x_itv)
        sgModel.set_min(x_itv_abs[1],0)
        sgModel.set_max(x_itv_abs[1],max(-l,u))
        return x_itv_abs[1]
    end
end
...
#这里新建一个函数用于设置sage的下最终输出约束的函数
function get_vars_for_max_index_withsage(
    sgModel::PyObject,
    xs_itv::Array{PyObject,1},
    xs::Array{<:JuMPLinearType,1},
    target_indexes::Array{<:Integer,1},
)::Tuple{PyObject,Array{<:PyObject,1}}

    @assert length(xs) >= 1
    @assert length(xs_itv) >= 1

    target_vars = xs[Bool[i ∈ target_indexes for i in 1:length(xs)]]
    nontarget_vars = xs[Bool[i ∉ target_indexes for i in 1:length(xs)]]

    target_vars_itv = xs_itv[Bool[i ∈ target_indexes for i in 1:length(xs_itv)]]
    nontarget_vars_itv = xs_itv[Bool[i ∉ target_indexes for i in 1:length(xs_itv)]]

    maximum_target_var_itv = length(target_vars_itv) == 1 ? target_vars_itv[1] : MIPVerify.maximum_itv(sgModel,target_vars,target_vars_itv)
    
    return maximum_target_var_itv,nontarget_vars_itv
end
...
function set_max_indexes_withsage(
    sgModel::PyObject,
    xs::Array{<:JuMPLinearType,1},
    xs_itv::Array{PyObject,1},
    target_indexes::Array{<:Integer,1};
    ...
    maximum_target_var, nontarget_vars = get_vars_for_max_index_withsage(sgModel,xs_itv, xs, target_indexes)
    
    
    sgModel.add_constraint(maximum_target_var - margin >= nontarget_vars)
