...
export find_adversarial_example, frac_correct, interval_arithmetic, lp, mip, sage_all
...
+ `tightening_algorithm::MIPVerify.TighteningAlgorithm`: Defaults to `mip`. Determines how we
    ...
    Allowed options are `interval_arithmetiching set_max_indexes(::Moion.
...
function find_adversarial_example(
    ...
    total_time = @elapsed begin
        ...
        println("the predictead index is : ",predicted_index)
        ...
        if !(d[:PredictedIndex] in d[:TargetIndexes]) || solve_if_predicted_in_targeted
            ...
            elseif adversarial_example_objective == worst
                ...
                # println("the lenght of maximum_target_var is: $(length(maximum_target_var))")
                # println("the lenght of nontarget_vars is: $(length(nontarget_vars))")
                maximum_nontarget_var = maximum_ge(nontarget_vars)
                # println("the lenght of maximum_nontarget_var is: $(length(maximum_nontarget_var))")
                ...
                # details.
                #@variable(m,v_obj>=0)
                v_obj = @variable(m, lower_bound = 0.0)
...
function robustness_checking_verification(
    nn::NeuralNet,
    input::Array{<:Real},
    target_selection::Union{Integer,Array{<:Integer,1}},
    optimizer,
    ...
    total_time = @elapsed begin
        ...
        # notice(
        #     MIPVerify.LOGGER,
        #     "Attempting to check robustness. Neural net predicted label is $(predicted_index), target labels are $(d[:TargetIndexes]) within robustness $radius",
        # )

        # Only call optimizer if predicted index is not found among target indexes.
        if !(d[:PredictedIndex] in d[:TargetIndexes]) || solve_if_predicted_in_targeted
            merge!(d, get_model_check_robust(nn, input, pp, optimizer, tightening_options, tightening_algorithm,radius))
            m = d[:Model]
